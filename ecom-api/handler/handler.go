...
func (h *handler) updateProduct(w http.ResponseWriter, r *http.Request) {
	...
	product, err := h.server.GetProduct(h.ctx, i)
	if err != nil {
		http.Error(w, "error getting prodyct", http.StatusInternalServerError)
		return
	}
...
func (h *handler) createOrder(w http.ResponseWriter, r *http.Request) {
	...
	createdOrder, err := h.server.CreateOrder(h.ctx, toStorerOrder(order))
	if err != nil {
...
func toStorerOrder(order OrderReq) *storer.Order {
	return &storer.Order{
		PaymentMethod: order.PaymentMethod,
		TaxPrice:      order.TaxPrice,
		ShippingPrice: order.ShippingPrice,
		TotalPrice:    order.TotalPrice,
		Items:         toStorerOrderItems(order.Items),
	}
}
...
func toOrderRes(order *storer.Order) OrderRes {
	return OrderRes{
		ID:            order.ID,
...
func (h *handler) listUser(w http.ResponseWriter, r *http.Request) {
	users, err := h.server.ListUser(h.ctx)
	if err != nil {
